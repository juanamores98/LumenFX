...
namespace LumenFX
{
    ...
    public static class FxModule
    {
        ...
        public static void ApplyOptimized() { if (!Presets.QuickPresets.ApplyOptimized()) throw new InvalidOperationException("OPTIMIZED could not be applied."); Flush(); }
        ...
        public static PanelView CreatePanel(UIComponent parent, float width = PreferredWidth, float height = 680f)
        {
            ...
            view.Number(page0, "Sun power (0 = map)", () => Runtime.TunerRuntime.CurrentState.SunPower, v => Edit(() => Runtime.TunerRuntime.CurrentState.SunPower = v), 0f, 20f, 0.01f);
            ...
            view.Number(page0, "Daylight gain", () => Runtime.TunerRuntime.CurrentState.SunStrength, v => Edit(() => Runtime.TunerRuntime.CurrentState.SunStrength = v), 0f, 3f, 0.01f);
            view.Number(page0, "Night gain", () => Runtime.TunerRuntime.CurrentState.MoonStrength, v => Edit(() => Runtime.TunerRuntime.CurrentState.MoonStrength = v), 0f, 2f, 0.01f);
            ...
            view.Number(page0, "Sky exposure (0 = map)", () => Runtime.TunerRuntime.CurrentState.SkyExposure, v => Edit(() => Runtime.TunerRuntime.CurrentState.SkyExposure = v), 0f, 5f, 0.001f);
            ...
            view.Number(page2, "Shadow bias scale", () => Runtime.TunerRuntime.CurrentState.BiasScale, v => Edit(() => Runtime.TunerRuntime.CurrentState.BiasScale = v), 0f, 2f, 0.01f);
            ...
            view.Number(page2, "Adaptive exposure gain", () => Runtime.TunerRuntime.CurrentState.AdaptiveExposureGain, v => Edit(() => Runtime.TunerRuntime.CurrentState.AdaptiveExposureGain = v), 0f, 1f, 0.01f);
            var presets = view.AddPage("Presets");
            var items = Presets.PresetLibrary.LoadAll();
            int selected = 0;
            string name = "My look";
            view.Choice(presets, "Saved look", () => items.Count == 0 ? new[] { "No saved looks" } : items.ConvertAll(p => p.Name).ToArray(), () => selected, v => selected = v);
            view.Action(presets, "Apply selected look", () => { if (selected >= 0 && selected < items.Count) Presets.PresetLibrary.Apply(items[selected]); });
            view.Text(presets, "Name", () => name, v => name = v);
            view.Action(presets, "Save look", () => { Presets.PresetLibrary.Save(Presets.PresetLibrary.Capture(name)); items = Presets.PresetLibrary.LoadAll(); });
        ...
        private static void Edit(Action edit)
        {
            ...
            Runtime.TunerRuntime.ApplyAll(); IO.StateStore.Save();
        }
